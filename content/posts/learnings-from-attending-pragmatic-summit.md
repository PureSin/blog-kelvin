...
I attended [Pragmatic Summit](https://www.pragmaticsummit.com/) this past Wednesday hosted by [Gergely Orosz](https://www.pragmaticengineer.com/). As with all things tech right now, the main (and really only) topic is AI. Thankfully the attendees are all active professionals working in tech, the discussions are grounded in hands-on experience with AI tools, avoiding much of the hype. Below is the summit agenda:

![Summit Agenda](/pragmatic_summit/summit_agenda.webp)

As a software engineer at Google working on the Photos mobile clients’ editing features, I’m lucky to not worry about the token budget in terms of costs and can freely use Gemini models and Google AI tools for my work. It was both scary and comforting to learn that engineers at other places are struggling with the same challenges as me right now. Mainly:
...
*Disclaimer: I work at Google but not on any AI model or tools. These are just my personal thoughts, and I'm as likely to be wrong as to be correct.*
...
AI writing code is the norm. It is now clear that AI, with tools like Codex/Claude Code/Gemini CLI, is capable of writing code (and even good/great code with detailed prompts) moving coding to a human language level.

And since AI doesn't get tired and can understand most common coding languages, the final step of translating detailed human intent into code is better left to AI. I'm still telling the AI what to write, but the level of specification depends on the task:
...
* Larger tasks that are novel or performance/security sensitive: more like a pair programming session, sitting with the AI tool and iterating.

My day to day now looks something like:


1. Triage my tasks into routine vs novel
...
Moving past the IC role, the larger question is how does a company adapt to this new world. Even if the frontier labs immediately pause on "smarter" models and focus on inference speed/model size/harness, the existing models will already lead to dramatic changes in processes. I’d like to end with some ideas (and a quote) presented in Laura Tacho’s talk (Data vs Hype). AI is here and its coding abilities are amazing, but engineers and people do more than code or move bytes on a computer, at the end of the day the value is in solving human problems.
